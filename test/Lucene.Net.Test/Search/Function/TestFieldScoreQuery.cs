...
namespace Lucene.Net.Search.Function
{
	...
	public class TestFieldScoreQuery:FunctionTestSetup
	{
		...
		private void  DoTestCaching(System.String field, FieldScoreQuery.Type tp)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				for (int j = 0; j < readers.Length; j++)
				{
					...
					try
					{
						if (i == 0)
						{
                            innerArray[j] = q.valSrc.GetValues(reader, null).InnerArray;
						...
						{
                            Log(i + ".  compare: " + innerArray[j] + " to " + q.valSrc.GetValues(reader, null).InnerArray);
                            Assert.AreSame(innerArray[j], q.valSrc.GetValues(reader, null).InnerArray, "field values should be cached and reused!");
						}
					}
			...
			ScoreDoc[] h2 = s.Search(q2, null, 1000, null).ScoreDocs;
			...
			for (int j = 0; j < readers2.Length; j++)
			{
				...
				try
				{
                    Log("compare: " + innerArray + " to " + q2.valSrc.GetValues(reader, null).InnerArray);
